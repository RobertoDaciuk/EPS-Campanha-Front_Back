...
export async function earningRoutes(
  ...
  // Hook para validação de valores monetários
  fastify.addHook('preHandler', async (request, reply) => {
    if ((request.method === 'POST' || request.method === 'PUT') && request.body) {
      const body = request.body as any;
      
      // Valida valores monetários se estiverem presentes
      if (body.amount !== undefined) {
        ...
        // Limite razoável para valores
        if (amount > 1000000) {
          reply.code(400).send({
            success: false,
            error: 'Valor muito alto',
            message: 'Valor excede o limite permitido (R$ 1.000.000)',
          });
          return;
        }
      }

      // Valida arrays de IDs em operações em lote
      if (body.earningIds && Array.isArray(body.earningIds)) {
        if (body.earningIds.length === 0) {
          reply.code(400).send({
            success: false,
            error: 'Lista vazia',
            message: 'Deve ser fornecido pelo menos um earning ID',
          });
          return;
        }
        
        if (body.earningIds.length > 100) {
          reply.code(400).send({
            success: false,
            error: 'Muitos itens',
            message: 'Máximo de 100 earnings por operação em lote',
          });
          return;
        }
      }
  ...
  // Hook para headers de auditoria
  fastify.addHook('onSend', async (request, reply, payload) => {
    // Adiciona headers de auditoria para operações financeiras
    const financialOperations = ['/mark-as-paid', '/cancel', '/bulk-process'];
    const isFinancialOp = financialOperations.some(op => request.url.includes(op));
    
    if (isFinancialOp) {
      reply.header('X-Financial-Operation', 'true');
      reply.header('X-Audit-Required', 'true');
      reply.header('X-Processed-By', request.user?.email || 'unknown');
      reply.header('X-Processed-At', new Date().toISOString());
    }

    // Cache específico para relatórios
    if (request.url.includes('/financial-report') || 
        request.url.includes('/projection') || 
        request.url.includes('/financial-summary')) {
      reply.header('Cache-Control', 'private, max-age=900'); // 15 minutos
    }

    // Dados pessoais de earning podem ter cache curto
    if (request.url.includes('/my-earnings')) {
      reply.header('Cache-Control', 'private, max-age=120'); // 2 minutos
    }
